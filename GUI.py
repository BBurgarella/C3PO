...
class MainApp(App):
    ...
    def build(self):
        ...
        for file in files:
            if ".json" in file:
                btn = Button(text=file[:-5], size_hint_y=None, size_hint_x=0.3, height=44, width=150)
                btn.bind(on_release=lambda btn=btn: dropdown.select(btn.text))
                dropdown.add_widget(btn)
        # Create the main button
        main_button = Button(text='Choose an agent', size_hint=(0.5, 0.5), height=44, width=300)
        ...
        for language, key in self.languages.items():
            btn = Button(text=language, size_hint_y=None, height=44, width=150)
            btn.bind(on_release=lambda btn=btn: lang_drop.select(btn.text))
            lang_drop.add_widget(btn)


        # Create the main button
        main_button_lang = Button(text='Choose a language', size_hint=(0.5, 0.5), height=44, width=300)
